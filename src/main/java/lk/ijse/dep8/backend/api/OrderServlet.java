...
import lk.ijse.dep8.backend.DTO.OrderItemDTO;
//import lk.ijse.dep8.backend.dto.IssueBookDTO;
...
@WebServlet(name = "IssueServlet", value = {"/order","/orders/"})
public class OrderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (Connection connection = pool.getConnection()) {
            ...
            String sql = "SELECT * FROM odr WHERE isbn LIKE ? OR nic LIKE ? OR date LIKE ? " + ((pagination) ? "LIMIT ? OFFSET ?" : "");

            PreparedStatement stm = connection.prepareStatement(sql);
            PreparedStatement stmCount = connection.prepareStatement("SELECT count(*) FROM odr WHERE isbn LIKE ? OR nic LIKE ? OR date LIKE ?");
            ...
            List<OrderItemDTO> issues = new ArrayList<>();

            while (rst.next()) {
                issues.add((new OrderItemDTO(
                        rst.getInt("id"),
                        rst.getString("nic"),
                        rst.getString("isbn"),
                        rst.getDate("date")
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try{
            ...
            Jsonb jsonb = JsonbBuilder.create();
            OrderItemDTO issue = jsonb.fromJson(req.getReader(), OrderItemDTO.class);
            ...
            try (Connection connection = pool.getConnection()) {
                ...
                PreparedStatement stm = connection.prepareStatement("SELECT * FROM item INNER JOIN member WHERE nic=? AND isbn=?");
                ...
                stm = connection.prepareStatement("SELECT * FROM odr WHERE isbn = ?");
                stm.setString(1, issue.getIsbn());
                if (stm.executeQuery().next()){
                    resp.sendError(HttpServletResponse.SC_GONE, "Item is not available");
                }

                /* Let's place the issue */
                issue.setDate(Date.valueOf(LocalDate.now()));
                stm = connection.prepareStatement("INSERT INTO odr (nic, isbn, date) VALUES (?,?,?)", Statement.RETURN_GENERATED_KEYS);
                stm.setString(1, issue.getNic());
